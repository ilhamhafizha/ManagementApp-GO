...
type BoardRepository interface {
	...
	FindAllByUserPaginated(userPublicID, filter, sort string, limit, ofset int) ([]models.Board, int64, error)
}
...
func (r *boardRepository) Update(board *models.Board) error {
	return config.DB.Model(&models.Board{}).
		Where("public_id = ?", board.PublicID).Updates(map[string]interface{}{"title": board.Title, "description": board.Description, "due_date": board.DueDate}).Error
}
...
func (r *boardRepository) FindAllByUserPaginated(userPublicID, filter, sort string, limit, ofset int) ([]models.Board, int64, error) {
	var board []models.Board
	var total int64
	...
	if err := query.Limit(limit).Offset(ofset).Find(&board).Error; err != nil {
		return nil, 0, err
	}
